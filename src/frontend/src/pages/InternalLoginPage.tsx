...
import { useGetCallerUser, useClaimSuperadmin, useValidateRoleName, useCheckSuperadminClaimed } from '@/hooks/useQueries';
...
function InternalLoginContent() {
  ...
  const { data: superadminClaimed, isLoading: checkingClaimed } = useCheckSuperadminClaimed();
  ...
  const showSuperadminClaim = isAuthenticated && isActorReady && !checkingClaimed && superadminClaimed === false;
  ...
  const handleWorkspaceClick = async (roleId: string) => {
    if (!isAuthenticated) {
      setErrorMessage('You must be logged in with Internet Identity first.');
      return;
    }
